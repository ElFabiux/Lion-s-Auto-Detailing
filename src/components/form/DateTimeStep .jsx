...
import { Calendar, Clock, ArrowRight, ArrowLeft, X, Check, Loader2, RefreshCw } from 'lucide-react';
...
const DateTimeStep = ({ data, updateData, onNext, onPrev, onExit }) => {
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      if (result.success) {
        ...
        const futureSlots = result.data.filter(slot => {
  ...
  const formatDateForDisplay = (isoDate) => {
    return formatDateForComparison(isoDate);
  };
      ...
      <div className={`transition-all duration-700 ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl">
          ...
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-white/60 text-sm">Consultando horarios disponibles...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl">
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <div className="text-red-400 text-center mb-6">
              <p className="text-lg font-semibold mb-2">No se pudo cargar la disponibilidad</p>
      ...
      <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/20 shadow-2xl">
        ...
        {availableSlots.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-white/60 text-lg mb-4">No hay horarios disponibles en este momento</p>
            <p className="text-white/40 text-sm">Por favor, contacte directamente para agendar su cita</p>
          </div>
        ) : (
          <>
              ...
              <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-red-orange-500/10 border border-red-orange-500/30 rounded-lg animate-fadeIn">
                <div className="text-center">
                  ...
                  <span className="text-white font-semibold text-sm sm:text-base">
                    {formatDateForDisplay(availableSlots[selectedSlot].fecha)} a las {availableSlots[selectedSlot].hora}
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="flex justify-between items-center mt-6 sm:mt-8 lg:mt-10 gap-3 sm:gap-4">
          ...
          <button
            onClick={handleNext}
            disabled={selectedSlot === null || availableSlots.length === 0}
